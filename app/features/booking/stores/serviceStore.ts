/**
 * Booking Feature — Service Store
 *
 * Canonical implementation moved from `app/stores/serviceStore.ts` as part of
 * Step 4 (migrate-booking-logic).  The legacy file at that path is now a
 * re-export shim and should be removed once all callers use this path.
 */
...
export const useServiceStore = defineStore('service-store', () => {
  ...
  async function fetchServiceProfessionals(serviceId: number) {
    const response = await api.get<ApiResponse<HealthProfessional[]>>(
      `api/services/${serviceId}/health_professionals`,
    )
    if (response.data) {
