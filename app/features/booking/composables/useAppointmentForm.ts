/**
 * Booking Feature — useAppointmentForm composable
 *
 * Canonical implementation moved from `app/composables/useAppointmentForm.ts`
 * as part of Step 4 (migrate-booking-logic).  The legacy file is now a
 * re-export shim.
 */
...
export function useAppointmentForm() {
  ...
  const { handleSubmit, isSubmitting, setFieldError, values, resetForm } =
  ...
  // Store State
  const { getServices: services, getServiceProfessionals: professionals } =
